...
- **Zakat** (`pages/ZakatPage.jsx`, route `/app/zakat`, reached from a Home quick action): nisab = value of 85g of 24k gold (editable gold price), Zakat = 2.5% of net zakatable wealth (cash + gold + investments + other − short debts) when it meets the nisab. It is a guiding estimate with a Sharia disclaimer + hawl note — keep both; don't present it as a fatwa. Inputs persist to `localStorage` (`tahseen.zakat.v1`).
