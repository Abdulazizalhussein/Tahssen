...
- **Analytics is reactive + chart-rich.** `AnalyticsPage` computes `computeHealth`/`computeForecast` and the chart series **synchronously in render** from the live account context, so the health score, deficit and charts update the instant income/expenses change — never load them async (that went stale). `AnalyticsCharts.jsx` has three SVG charts fed by `lib/finance` series: a spending trajectory (actual vs projected vs budget line, RTL day axis), seasonal monthly bars (`monthlySpendSeries` — Saudi Ramadan/Eid/summer peaks, marks the peak + average), and a fixed-expense category donut. Charts are RTL-aware (x-axis flips) and respect `prefers-reduced-motion`.
